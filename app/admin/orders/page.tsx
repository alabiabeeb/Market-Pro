  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-[#0A2E1A]">Orders</h1>
              ...
              {paginated.length===0 ? (
                <tr><td colSpan={6} className="px-5 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <button onClick={()=>{ setSearch(""); setStatusFilter("All"); }} className="text-xs text-[#0A2E1A] hover:underline">Clear filters</button>
                  </div>
                </td></tr>
              ) : paginated.map(o=>(
                <tr key={o.id} className="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors group cursor-pointer" onClick={()=>setViewOrder(o)}>
                  <td className="px-5 py-3.5 text-[#0A2E1A] font-semibold text-xs">{o.id}</td>
          ...
          <div className="flex items-center gap-1">
            ...
            {Array.from({length:totalPages},(_,i)=>i+1).map(p=>(
              <button key={p} onClick={()=>setCurrentPage(p)}
                className={`w-7 h-7 text-xs font-medium rounded-lg transition-colors ${currentPage===p?"bg-[#0A2E1A] text-[#C8F135]":"border border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800"}`}>{p}</button>
            ))}
